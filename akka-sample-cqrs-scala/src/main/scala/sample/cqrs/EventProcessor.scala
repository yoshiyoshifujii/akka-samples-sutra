...
object EventProcessor {
  ...
  abstract class EventProcessorStream[Event: ClassTag](
    ...
    private val query = PersistenceQuery(system.toClassic).readJournalFor[CassandraReadJournal](CassandraReadJournal.Identifier)
    ...
    private def processEventsByTag(offset: Offset): Source[Offset, NotUsed] =
      query.eventsByTag(tag, offset).mapAsync(1) { eventEnvelope =>
        eventEnvelope.event match {
          case event: Event =>
            processEvent(event, PersistenceId.ofUniqueId(eventEnvelope.persistenceId), eventEnvelope.sequenceNr).map(_ => eventEnvelope.offset)
          case other =>
        ...
        .withBackoff(minBackoff = 500.millis, maxBackoff = 20.seconds, randomFactor = 0.1) { () =>
          Source.futureSource {
            readOffset().map { offset =>
              log.info("Starting stream for tag [{}]", tag, offset)
