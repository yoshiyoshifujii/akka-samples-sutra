package com.github.yoshiyoshifujii.akka.samples.infrastructure.ulid
...
object ULID {
  ...
  def parseFromString(text: String): Try[ULID] =
    Try {
...
final case class ULID private (private val value: Value) extends Ordered[ULID] {
  ...
  override def compare(that: ULID): Int = value.compareTo(that.value)
